...
export const softDeleteAddress = asyncHandler(async (req, res) => {
    ...
    const address = await UserAddressModel.findOneAndUpdate({ _id: new mongoose.Types.ObjectId(addressId), userId: new mongoose.Types.ObjectId(userId) }, { $set: { isDeleted: true } });
    if (!address) {
