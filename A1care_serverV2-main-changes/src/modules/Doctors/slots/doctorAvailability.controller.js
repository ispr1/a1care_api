...
export const getDoctorAvailabilitybyDoctorId = asyncHandler(async (req, res) => {
    ...
    const doctorAvailability = await doctorAvailabilityModel.find({ doctorId: new mongoose.Schema.Types.ObjectId(doctorId) });
...
export const availableSlotByDoctorId = asyncHandler(async (req, res) => {
    ...
    const { startingTime, endingTime, slotDuration } = checkingSlot;
    ...
    (isToday
        ...
        !busySlots.some(busy => slot.startSlot < busy.endingTime &&
    ...
    return res.json(new ApiResponse(200, "Available Slots", readableSlots));
});
//# sourceMappingURL=doctorAvailability.controller.js.map