...
export const getStaffByRoleId = asyncHandler(async (req, res) => {
    ...
    const roleIds = roleId.split(',').map(id => id.trim());
...
export const sendOtpForStaff = asyncHandler(async (req, res) => {
    ...
    await RedisClient.setEx(`otp:${otpSessionId}`, 300, JSON.stringify({
        otpHash,
        mobileHash,
        attempts: 0,
        expiresAt: Date.now() + 300_000
    }));
    return res.status(200).json(new ApiResponse(200, "OTP sent to your mobile number", { otpSessionId, otp }));
});
...
export const verifyOtp = asyncHandler(async (req, res) => {
    ...
    const token = jwt.sign({ staffId: staff._id }, process.env.JWT_SECRET, { expiresIn: "7d" });
...
export const getStaffDetials = asyncHandler(async (req, res) => {
    ...
    if (!staffId)
        throw new ApiError(401, "Not authorzed to access");
...
export const checkOtpStatus = asyncHandler(async (req, res) => {
    ...
    if (!otpSession)
        throw new ApiError(404, "Session exprired or invalid");
