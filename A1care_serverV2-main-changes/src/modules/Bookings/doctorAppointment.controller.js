...
export const getAppointmentsByPatientId = asyncHandler(async (req, res) => {
    const patientId = req.user?.id;
    const appointments = await doctorAppointmentModel.find({ patientId: patientId }).populate('doctorId').populate('patientId');
    return res.status(200).json(new ApiResponse(200, "Appointments fetched", appointments));
});
//# sourceMappingURL=doctorAppointment.controller.js.map